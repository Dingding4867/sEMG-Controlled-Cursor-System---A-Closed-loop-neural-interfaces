...
elapsed_time = nan(1,RUN_LENGTH); %FILLIN. time it takes loop to execute [time you will run your loop x 1]
...
for i= 1:RUN_LENGTH%FILLIN
    ...
    emg_inputs(:,i) = mean(abs(data_chunk), 2); %FILLIN
    ...
    if i == BASELINE_ESTIMATE_LENGTH%%FILLIN (some eqality)
        ch_offsets = min(emg_inputs(:, 1:BASELINE_ESTIMATE_LENGTH), [], 2); %FILLIN. see top of script/lab manual 
        ch_scale   = EMG_SCALE *1./(max(emg_inputs(:, 1:BASELINE_ESTIMATE_LENGTH),[], 2)-ch_offsets); %FILLIN. see top of script/lab manual
         
        disp('Re-scaling EMG data complete') 
    end
    emg_inputs(:,i) = ch_scale .* (emg_inputs(:,i) - ch_offsets);
    ...
    %hint: look at the help for 'hist'. 
    ...
    elapsed_time(i) = etime(clock, t_0)*1000; %FILLIN %FILLIN
end
